...
//							myControl.util.dump(" -> extension "+namespace+" loaded fine but contained no callback");
		...
		executeExtensionCallback : function(namespace,callback)	{
			myControl.util.dump("BEGIN model.executeExtensionCallback ["+namespace+"]");
			if(typeof callback == 'function'){eval(callback)}
			else if(typeof callback == 'string' && myControl.ext[namespace] && typeof myControl.ext[namespace].callbacks[callback].onSuccess == 'function')	{myControl.ext[namespace].callbacks[callback].onSuccess()}
...
//			myControl.util.dump("BEGIN model.handleDependenciesFor");
...
//			myControl.util.dump(" -> after allTemplatesHaveLoaded. pass: "+pass);
			...
			if(pass)	{
				myControl.util.dump(" -> all dependencies for extension: "+namespace+" should be loaded. execute callback.");
				myControl.model.executeExtensionCallback(namespace,callback);
				}
			else	{
