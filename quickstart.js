...
//if pageType isn't passed in, we're likely in a popState, so look in infoObj.
				...
				if(pageType == 'checkout'){
					$('.checkout-footer a[href="#checkout?show=checkout"]').hide();
					}
				else {
					$('.checkout-footer a[href="#checkout?show=checkout"]').show();
					}
...
//these will change the cursor to 'wait' and back to normal as each template loads/finishes loading.
					...
					myControl.ext.myRIA.template[pageTemplates[i]].onCompletes.push(function(P){myControl.ext.myRIA.util.changeCursor('auto')});
					}
